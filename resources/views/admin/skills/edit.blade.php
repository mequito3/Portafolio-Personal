...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center gap-4 mb-10">
        ...
        <div>
            <h1 class="text-3xl font-display font-bold text-white tracking-tight mb-1">Editar <span class="gradient-text">Habilidad</span></h1>
            <p class="text-gray-500 text-sm font-medium uppercase tracking-widest">Actualizando: {{ $skill->name }}</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.skills.update', $skill) }}" class="space-y-10 pb-20">
        @csrf
        @method('PUT')
