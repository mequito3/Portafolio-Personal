    ...
    <div class="flex items-center gap-4 mb-10">
        ...
        <div>
            <h1 class="text-3xl font-display font-bold text-white tracking-tight italic">
                Editar <span class="gradient-text">Proyecto</span>
            </h1>
            <p class="text-gray-500 text-[10px] font-bold uppercase tracking-[2px] mt-1 ml-0.5">Modificando: {{ $project->title }}</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.projects.update', $project) }}" enctype="multipart/form-data" class="space-y-10 pb-20">
        @csrf
        @method('PUT')
